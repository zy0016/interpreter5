...
//////////////////////////////////////////////////////////
OTPERATOR_TYPE cVariableInfo::Precede(char operator_old,char operator_new)//运算符优先级比较
{
    switch (operator_old)
    {
    case '(':
...
int cVariableInfo::JudgeOperator(char copeator_new,const char * pLineData)
{
    ...
    {
        if (pStackNumberList->GetElementAmount() == 0)//数值栈中没有数值,这是个类似于 +5或者-9的情况
        {
            pStackNumberList->ElementPush(0);//数值栈入"0"作为第一个数据

            printf("t%d = %d\r\n",iVariableNum++,0);
        }
        else
        {
            if (pStackOperatorList->GetOperatorArithmetic() + 1 > pStackNumberList->GetElementAmount())//运算符栈大于数值栈,类似于-(+3 * 6)的情况
            {
    ...
    {
        if (pStackNumberList->GetElementAmount() == 0)//数值栈中没有数值,这是个类似于 "*5"或者"/9"的情况,认为表达式非法
        {
            RemoveNewLine(pLineData,ErrorString,sizeof(ErrorString));
    ...
    if (pStackOperatorList->ElementGetTop(&copeator_old) == 0)//栈为空
    {
    ...
    else//符号栈不为空
    {
        ...
        case OTPERATOR_HIGH:
            ...
            pStackOperatorList->ElementPull(NULL);//运算符出栈
            if (pStackOperatorList->ElementGetTop(&coperator_temp) == 1)//取得栈顶运算符
            {
            ...
            if (copeator_new == '+' || copeator_new == '-' || copeator_new == '*' || copeator_new == '/')
                pStackOperatorList->ElementPush(copeator_new);//新的运算符重新入栈
            break;
        case OTPERATOR_LOW:
            pStackOperatorList->ElementPush(copeator_new);//运算符入栈
            break;
...
int cVariableInfo::JudgeNewLine(const char * pLineData)
{
    ...
    if (pStackOperatorList->GetElementAmount())//表达式尚未运算完毕
    {
...
int cVariableInfo::CountCurrentValue(int icurvalue,int * presult)
{
    ...
    if (pStackOperatorList->ElementGetTop(&coperator_temp))
    {
        ...
        if (coperator_temp == '*' || coperator_temp == '/')
        {
            ...
            pStackOperatorList->ElementPull(NULL);//退掉前一个运算符
            * presult = ivalue;
            return 1;
        }
    }
